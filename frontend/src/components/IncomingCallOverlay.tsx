...
interface IncomingCallOverlayProps {
  onCallActive: (sessionId: string, pc: RTCPeerConnection, stream: MediaStream) => void;
}

export default function IncomingCallOverlay({ onCallActive }: IncomingCallOverlayProps) {
  ...
  // Show overlay when there's an incoming VOICE call for this user
  useEffect(() => {
    if (
      activeSession &&
      activeSession.status === CallStatus.calling &&
      activeSession.callType === CallType.voice &&
  ...
  const handleAccept = useCallback(async () => {
    ...
    try {
      // 1. Get user media (audio only for voice call)
      ...
      setVisible(false);
      setAccepting(false);
      onCallActive(activeSession.id, pc, stream);
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex flex-col items-center justify-center bg-rose-dark/95 backdrop-blur-md animate-in fade-in duration-300">
      {/* Decorative hearts */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        {[...Array(5)].map((_, i) => (
          <Heart
            key={i}
            className="absolute text-rose-pink/20 animate-pulse"
            fill="currentColor"
            style={{
              width: `${20 + i * 10}px`,
              height: `${20 + i * 10}px`,
      ...
      <div className="relative flex flex-col items-center gap-6 px-8 text-center">
        ...
        <div className="flex items-center gap-2">
          <Phone className="w-4 h-4 text-rose-pink/80 animate-pulse" />
        ...
        <div className="relative">
          <div className="absolute inset-[-12px] rounded-full bg-warm-accent/20 animate-ping" />
          <div className="absolute inset-[-6px] rounded-full bg-warm-accent/15 animate-pulse" />
        ...
        <div>
          ...
          <p className="text-rose-pink/70 mt-1 text-sm">is calling you…</p>
        </div>
